...
public class DatabaseHandler {
    ...
    /**
     * Create command_usage table for tracking bot command usage
     */
    private void createCommandUsageTable() throws SQLException {
        String createTable = "CREATE TABLE IF NOT EXISTS command_usage (" +
            "id INTEGER PRIMARY KEY AUTOINCREMENT, " +
            "guild_id INTEGER, " +
            "user_id INTEGER, " +
        ...
        stmt.execute("CREATE INDEX IF NOT EXISTS idx_command_usage_guild_time ON command_usage(guild_id, executed_at)");
        stmt.execute("CREATE INDEX IF NOT EXISTS idx_command_usage_user_time ON command_usage(user_id, executed_at)");
        stmt.execute("CREATE INDEX IF NOT EXISTS idx_command_usage_command_time ON command_usage(command_name, executed_at)");
    }
    ...
    // ========== Command Usage Statistics Methods ==========
    
    /**
     * Log a command usage event
     */
    public void logCommandUsage(String guildId, String userId, String commandName, String commandOptions, 
                               String channelId, long executionTimeMs, boolean success, String errorMessage) {
        try {
            String insertQuery = "INSERT INTO command_usage (guild_id, user_id, command_name, command_options, channel_id, execution_time_ms, success, error_message) " +
                               "VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
            ...
            stmt.setString(4, commandOptions);
            ...
            stmt.setInt(7, success ? 1 : 0);
            stmt.setString(8, errorMessage);
            
            stmt.executeUpdate();
        } catch (SQLException e) {
            System.err.println("Error logging command usage: " + e.getMessage());
            e.printStackTrace();
        }
    }
     ...
     */
    public EmbedBuilder getBotUsageStatisticsEmbed(String guildId, int days) {
        EmbedBuilder embed = new EmbedBuilder()
                .setTitle("🤖 Bot Usage Statistics")
                .setDescription(String.format("Comprehensive usage statistics for the last %d days", days))
                .setColor(Color.CYAN)
                .setTimestamp(java.time.Instant.now());

        try {
            String dateCondition = days > 0 ? "AND DATE(executed_at) >= DATE('now', '-" + days + " days')" : "";
            
            // Most used commands
            String topCommandsQuery = "SELECT command_name, COUNT(*) as usage_count FROM command_usage " +
                                    "WHERE (guild_id = ? OR guild_id IS NULL) " + dateCondition + 
                                    " GROUP BY command_name ORDER BY usage_count DESC LIMIT 10";
            PreparedStatement topCommandsStmt = connection.prepareStatement(topCommandsQuery);
            topCommandsStmt.setString(1, guildId);
            ResultSet topCommandsRs = topCommandsStmt.executeQuery();
            
            StringBuilder topCommands = new StringBuilder();
            int rank = 1;
            while (topCommandsRs.next() && rank <= 10) {
                String commandName = topCommandsRs.getString("command_name");
                int count = topCommandsRs.getInt("usage_count");
                topCommands.append(String.format("%d. `/%s` - %d uses\n", rank++, commandName, count));
            }
            
            if (topCommands.length() > 0) {
                embed.addField("📈 Most Used Commands", topCommands.toString(), false);
            }
            
            // User activity (only for guild commands)
            if (guildId != null && !guildId.isEmpty()) {
                String topUsersQuery = "SELECT user_id, COUNT(*) as command_count FROM command_usage " +
                                     "WHERE guild_id = ? " + dateCondition + 
                                     " GROUP BY user_id ORDER BY command_count DESC LIMIT 5";
                PreparedStatement topUsersStmt = connection.prepareStatement(topUsersQuery);
                topUsersStmt.setString(1, guildId);
                ResultSet topUsersRs = topUsersStmt.executeQuery();
                
                StringBuilder topUsers = new StringBuilder();
                rank = 1;
                while (topUsersRs.next() && rank <= 5) {
                    String userId = topUsersRs.getString("user_id");
                    int count = topUsersRs.getInt("command_count");
                    topUsers.append(String.format("%d. <@%s> - %d commands\n", rank++, userId, count));
                }
                
                if (topUsers.length() > 0) {
                    embed.addField("👥 Most Active Users", topUsers.toString(), false);
                }
            }
            ...
            String totalStatsQuery = "SELECT " +
                                   ...
                                   "FROM command_usage " +
                                   "WHERE (guild_id = ? OR guild_id IS NULL) " + dateCondition;
            PreparedStatement totalStatsStmt = connection.prepareStatement(totalStatsQuery);
            totalStatsStmt.setString(1, guildId);
            ResultSet totalStatsRs = totalStatsStmt.executeQuery();
            
            if (totalStatsRs.next()) {
                int totalCommands = totalStatsRs.getInt("total_commands");
                int uniqueUsers = totalStatsRs.getInt("unique_users");
                int uniqueCommandTypes = totalStatsRs.getInt("unique_commands");
                double avgExecutionTime = totalStatsRs.getDouble("avg_execution_time");
                int failedCommands = totalStatsRs.getInt("failed_commands");
                double successRate = totalCommands > 0 ? ((double)(totalCommands - failedCommands) / totalCommands) * 100 : 0;
                
                StringBuilder generalStats = new StringBuilder();
                generalStats.append(String.format("**Total Commands:** %d\n", totalCommands));
                generalStats.append(String.format("**Unique Users:** %d\n", uniqueUsers));
                generalStats.append(String.format("**Command Types Used:** %d\n", uniqueCommandTypes));
                generalStats.append(String.format("**Average Response Time:** %.1fms\n", avgExecutionTime));
                generalStats.append(String.format("**Success Rate:** %.1f%%\n", successRate));
                generalStats.append(String.format("**Failed Commands:** %d", failedCommands));
                
                embed.addField("📊 General Statistics", generalStats.toString(), false);
            }
            
            // Daily activity (last 7 days only for readability)
            int activityDays = Math.min(days, 7);
            String dailyStatsQuery = "SELECT DATE(executed_at) as date, COUNT(*) as daily_count FROM command_usage " +
                                   "WHERE (guild_id = ? OR guild_id IS NULL) AND DATE(executed_at) >= DATE('now', '-" + activityDays + " days') " +
                                   "GROUP BY DATE(executed_at) ORDER BY date DESC";
            PreparedStatement dailyStatsStmt = connection.prepareStatement(dailyStatsQuery);
            dailyStatsStmt.setString(1, guildId);
            ResultSet dailyStatsRs = dailyStatsStmt.executeQuery();
            
            StringBuilder dailyActivity = new StringBuilder();
            while (dailyStatsRs.next()) {
                String date = dailyStatsRs.getString("date");
                int count = dailyStatsRs.getInt("daily_count");
                dailyActivity.append(String.format("**%s:** %d commands\n", date, count));
            }
            
            if (dailyActivity.length() > 0) {
                embed.addField("📅 Daily Activity (Last " + activityDays + " Days)", dailyActivity.toString(), false);
            }
            
        } catch (SQLException e) {
            System.err.println("Error getting bot usage statistics: " + e.getMessage());
            e.printStackTrace();
            embed.addField("Error", "Failed to retrieve usage statistics", false);
        }
